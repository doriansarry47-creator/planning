...
// Utiliser conditionnellement le bon schéma selon la base de données
const isSqlite = process.env.DATABASE_URL?.startsWith("file:");

// Import dynamique du schéma approprié
const getSchemas = async () => {
  if (isSqlite) {
    const sqliteSchema = await import("../../shared/schema-sqlite");
    return {
      appointments: sqliteSchema.appointments,
      patients: sqliteSchema.patients,
      practitioners: sqliteSchema.practitioners,
      availabilitySlots: sqliteSchema.availabilitySlots,
      insertAppointmentSchema: sqliteSchema.insertAppointmentSchema,
    };
  } else {
    const pgSchema = await import("../../shared/schema");
    return {
      appointments: pgSchema.appointments,
      patients: pgSchema.patients,
      practitioners: pgSchema.practitioners,
      availabilitySlots: pgSchema.availabilitySlots,
      insertAppointmentSchema: pgSchema.insertAppointmentSchema,
    };
  }
};
...
router.get("/patient", authMiddleware(['patient']), async (req, res) => {
  try {
    ...
    const { appointments, practitioners } = schemas;
...
router.post("/", authMiddleware(['patient']), async (req, res) => {
  try {
    ...
    const schemas = await getSchemas();
    const { appointments, patients, practitioners, insertAppointmentSchema } = schemas;
    const validatedData = insertAppointmentSchema.parse(appointmentData);
    
    // Pour SQLite, utiliser une approche simplifiée sans transaction complexe
    let result: any;
    ...
    if (isSqlite) {
      ...
      const newAppointment = await db.select().from(appointments).where(eq(appointments.id, appointmentId)).limit(1);
      result = newAppointment[0];
    } else {
      // Transaction PostgreSQL pour éviter les double-bookings
